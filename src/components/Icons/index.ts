/**
 * Icons - Reusable SVG icon components
 *
 * Centralized icon library for consistent usage across the application.
 * All icons accept className and aria-label props for customization and accessibility.
 *
 * @example
 * ```tsx
 * import { InfoIcon, CloseIcon, ChevronIcon } from '@/components/Icons';
 *
 * <InfoIcon className="w-4 h-4 text-gray-500" />
 * <CloseIcon className="w-6 h-6" aria-label="Close dialog" />
 * <ChevronIcon direction="down" className="w-4 h-4" />
 * ```
 */

export { InfoIcon } from "./InfoIcon";
export { CloseIcon } from "./CloseIcon";
export { ChevronIcon } from "./ChevronIcon";
